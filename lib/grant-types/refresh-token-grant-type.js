...
RefreshTokenGrantType.prototype.handle = function(request, client) {
  ...
  return Promise.bind(this)
    ...
    .then(function(token) {
      return this.saveToken(token, client);
    });
};
...
RefreshTokenGrantType.prototype.saveToken = function(oldToken, client) {
  var fns = [
    this.generateAccessToken(client, oldToken.user, oldToken.scope),
    this.generateRefreshToken(client, oldToken.user, oldToken.scope),
  ...
  return Promise.all(fns)
    ...
    .then(function(token) {
      return promisify(this.model.saveToken, 3).call(this.model, token, client, oldToken.user)
